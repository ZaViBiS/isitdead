...
func NewTelegramSender(store TelegramAccountStore, endpoint, secret string) *TelegramSender {
...
func (s *TelegramSender) Send(ctx context.Context, message Message) error {
	if s.endpoint == "" {
		return fmt.Errorf("telegram integration endpoint is not configured")
	}

	account, err := s.store.GetTelegramAccountByUserID(message.User.ID)
	if err != nil {
		return err
	}
...
func telegramText(message Message) string {
	if message.SSLStatus != nil {
		...
		return fmt.Sprintf(
			"isitdead.cc: SSL certificate reminder\n\n%s\nURL: %s\nExpires in: %d days\nExpires at: %s\nChecked at: %s",
			message.Server.Name,
			message.Server.URL,
